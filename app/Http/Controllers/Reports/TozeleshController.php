...
class TozeleshController extends Controller
{
    public function getReport($id = null, Request $request){
        ...
        $data['view_type'] = ($request->view_type == 0) ? ['id' => 0, 'view' => 'table'] : ['id' => 1, 'view' => 'chart'];
        $data['cur_id'] = $id;
        ...
        $meters = Meter::with('address.parentRecursive', 'rate')->joinAddress()->where('tozelesh', 1)
        ...
        if($data['view_type']['id'] == 1){
            $date_start = Carbon::parse($data['date_from']);
            $date_end = Carbon::parse($data['date_to']);
            $time_start = Carbon::parse($data['time_from']);
            $time_end = Carbon::parse($data['time_to']);

            switch ($data['current_frame']){
                case 1:{
                    for($dt = $date_start->day(1); $date_start->lte($date_end); $dt->addMonth()){
                        $data['dates'][] = $dt->format('Y-m-d 00:00:00');
                    }
                    break;
                }
                case 2:{
                    for($dt = $date_start; $date_start->lte($date_end); $dt->addDay()){
                        $data['dates'][] = $dt->format('Y-m-d 00:00:00');
                    }
                    break;
                }
                case 3:{
                    for($dt = $time_start; $time_start->lte($time_end); $dt->addHour()){
                        $data['dates'][] = $dt->format('Y-m-d H:00:00');
                    }
                    break;
                }
                case 4:{
                    for($dt = $time_start; $time_start->lte($time_end); $dt->addMinutes(15)){
                        $data['dates'][] = $dt->format('Y-m-d H:i:00');
                    }
                    break;
                }
            }
        ...
        return view('reports.tozelesh', $data);
    }
}
