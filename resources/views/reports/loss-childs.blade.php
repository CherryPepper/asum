@php
    ...
    if(!empty($parents)){
        foreach ($parents as $parent)
            $breadcrumbs[] = [$parent->serial, route('report.loss.childs', ['id' => $parent->id])];

        $breadcrumbs[] = ['Пользовательские счетчики'];
    }
@endphp
                ...
                <div class="col-sm-12">
                    <div class="panel">
                        ...
                        <div class="panel-body">
                            <form action="{{route('report.loss.childs', ['id' => $parent_id])}}">
                                <div class="col-sm-11">
                                    <div class="row">
                                        ...
                                        <div class="row">
                                            ...
                                            <div class="form-wrapper col-sm-2">
                                                <label></label>
                                                <a type="submit" class="btn btn-default btn-block input-lg send-form mt-4" href="{{route('report.loss.childs', ['id' => $parent_id])}}">Очистить</a>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                            ...
                            <div class="table-responsive">
                                <table class="table table-striped table-bordered table-hover report">
                                    ...
                                    <tr>
                                        ...
                                        <th>
                                            Не опрошенных
                                            <span class="badge" data-toggle="tooltip" data-placement="top" title="Количество не опрошенных точек в заданном интервале"> ? </span>
                                        </th>
                                        <th>
                                            С ошибкой
                                            <span class="badge" data-toggle="tooltip" data-placement="top" title="Количество точек в заданном интервале вернувших ошибку"> ? </span>
                                        </th>
                                        <th>
                                            Потребление КВТ
                                            <span class="badge" data-toggle="tooltip" data-placement="top" title="Потребление энергии в заданном интервале"> ? </span>
                                        </th>
                                        <th>Действия</th>
                                    </tr>
                                    </thead>
                                    ...
                                    @if(!empty($meters->total()))
                                        @foreach($meters as $meter)
                                            <tr>
                                                ...
                                                <td>{{$meter->without_request}}</td>
                                                <td>{{$meter->with_error}}</td>
                                                <td>{{(float)$meter->sum_difference}}</td>
                                                <td class="actions">
